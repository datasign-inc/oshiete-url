...
当社は、本サービスの提供、品質改善、分析および不正利用防止等のために、利用者が本サービスを利用する際に、以下の情報を自動的に収集します。
なお、当社による利用者情報の取扱いについては、別途定める「[教えてURL プライバシーポリシー](https://oshiete-url.jp/privacy.html)」に従うものとします。

1. アクセスしたURLおよびドメインの文字列
利用者がブラウザでアクセスしたウェブページのURL、ドメイン名その他それに付随する文字列情報。

2. アクセスしたURLで検出されたサービスに関する情報
本サービスが分析により検出した、アクセス先ウェブページ上で利用されている外部サービス、タグ、技術要素その他の関連情報。

3. アクセスしたURLにおいて発生しているサードパーティへのHTTP通信に含まれるURL情報
利用者がアクセスしたウェブページから送信されるサードパーティ宛のHTTPリクエストに含まれるURL等の情報。
